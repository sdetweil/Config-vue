<template>
	<!--
  Generated template for the FileModalPage page.
	...
	<ion-content padding width="100%">
		...
		<ion-row
			*ngFor="let file of files; let i = index"
			@click="setClickedRow(i,'file','any')"
			(press)="doubleClick(i,file)"
			(dblTap)="doubleClick(i,file)"
		>
			<ion-col col-2 text-center class="colb " no-padding>
				<!--  -->
				<ion-checkbox
					v-bind="file.selected"
					@click="setClickedRow(i,'file','any')"
				></ion-checkbox>
			</ion-col>
			<ion-col col-2 class="colb center">{{ file.filetype }}</ion-col>
			<ion-col col-8 class="colb" no-padding>{{ file.name }}</ion-col>
		...
		<ion-row>
			<ion-col col-2 alignleft>
				<button @click="closeModal()">
					Cancel <ion-icon md="md-close" ios="ios-close"></ion-icon>
				</button>
			</ion-col>
			<ion-col col-8> </ion-col>
			<ion-col col-2>
				<button @click="saveModal()" float-right>
					<ion-icon md="md-save" ios="ios-save"></ion-icon>Save
				</button>
			</ion-col>
		</ion-row>
	</ion-footer>
